...
DIMENSION_METADATA = {'lat': {'long_name': 'latitude',
...
def write_single_band_output(target_area: AreaDefinition,
    ...
    with Dataset(variable_output_path, 'w', format='NETCDF4') as output_file:
        dimensions = write_dimensions(output_file, target_area,
                                      reprojection_cache)
        grid_mapping_name = write_grid_mapping(output_file, target_area,
                                               dimensions)
        write_science_variable(output_file, reprojected_data, variable_name,
                               dimensions, grid_mapping_name, attributes)
...
def write_grid_mapping(dataset: Dataset, target_area: AreaDefinition,
...
def write_science_variable(dataset: Dataset, data_values: np.ndarray,
                           variable_full_name: str, dimensions: Tuple[str],
                           grid_mapping_name: str, attributes: Dict) -> None:
    ...
    """
    variable = dataset.createVariable(variable_full_name, data_values.dtype,
...
def write_dimension_variables(dataset: Dataset, dimensions: Tuple[str],
    ...
    for dimension_name, dimension_vector in dimension_data.items():
        variable = dataset.createVariable(dimension_name,
                                          dimension_vector.dtype,
                                          dimensions=(dimension_name,))

        variable[:] = dimension_vector

        attributes = next(attributes
                          for coordinate, attributes
                          in DIMENSION_METADATA.items()
