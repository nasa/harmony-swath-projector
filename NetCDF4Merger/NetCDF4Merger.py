...
""" Merging reprojected single-dataset netCDF4 files from GDAL into one, and copy attributes from the original input
    file
"""
def create_output(inputfile, outputfile, temp_dir):
        ...
        for file in files:
            ...
            for name, var in data.variables.items():
                if name not in list(out.variables.keys()):
                    ...
                    if name != GDAL_DATASET_NAME: copy_variable(inf, out, data, name)
                    else: copy_variable(inf, out, data, dataset_name)
...
def copy_variable(inf, out, rep, dataset_name):
